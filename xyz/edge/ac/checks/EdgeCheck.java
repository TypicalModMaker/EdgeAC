    ...
    public void fixGhostBlocks() {
        Bukkit.getScheduler().runTask(Edge.getInstance(), () -> {
            synchronized (this.user.getMovementHandler().getBlocks()) {
                this.user.getMovementHandler().getBlocks().iterator();
                ...
                for (final Pair<Integer, String> punish : this.commandVlMap) {
                    if (punish.getX() == this.vl) {
                        Bukkit.getScheduler().runTask(Edge.getInstance(), () -> {
                            ...
                            punish.getY().replaceAll("%player%", this.user.getPlayer().getName()).replace("%check%", this.customCheckName.isEmpty() ? ColorUtil.translate(this.justTheName) : this.customCheckName);
                            String replacement;
                            ...
                            Bukkit.dispatchCommand((CommandSender)o, s.replace(target, replacement));
                        });
                    }
                }
            }
