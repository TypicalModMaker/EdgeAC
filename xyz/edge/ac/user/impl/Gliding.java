...
import xyz.edge.ac.packetevents.packetwrappers.play.out.entitymetadata.WrappedWatchableObject;
        ...
        if (entityMetadata.getEntityId() == this.user.getPlayer().getEntityId()) {
            if (ServerVersion.getVersion().isNewerThanOrEquals(ServerVersion.v_1_14)) {
                ...
                metadata.removeIf(element -> {
                    new WrappedPacket(new NMSPacket(element));
                    final WrappedPacket wrappedPacket;
                    final Object dataWatcherObject = wrappedPacket.readAnyObject(0);
                    new WrappedPacket(new NMSPacket(dataWatcherObject));
                    final WrappedPacket wrappedPacket2;
